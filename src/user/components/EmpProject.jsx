...
import AddEmp from "../../admin/components/AddEmp";
...
const EmpProject = () => {
  ...
  return (
    <div className="h-[90rem] w-full flex  items-center  flex-col">
      <MainContainer />
      <div className="flex mt-10 w-full justify-center">
        <PageInfo pageName={"Project/Work"} />
      </div>
      <header className="flex w-[90%] justify-between items-center ">
        <h2>Projects</h2>
        <button
      ...
      <main className="flex justify-between gap-2 w-[90%]">
          ...
          <div className="border h-fit w-full rounded-md">
            ...
            <div className="m-3">
              <h4 className="font-semibold flex  items-center h-0 w-full justify-between">
                Progress: <p className="text-[#4bd765]"> {value}%</p>
              </h4>
            </div>
            <div className="relative mb-6 ml-5">
              <div
                className="absolute left-0 right-0 flex justify-center bottom-4"
                style={{ marginLeft: `${value}%` }}
              >
                <span className="bg-gray-700 text-white py-1 px-3 rounded hover:block ">
                  {value}%
                </span>
              </div>
              <input
                type="range"
                className="w-[90%] h-2 bg-green-500 rounded-lg appearance-none cursor-pointer dark:bg-gray-700"
                min="0"
                max="100"
                value={value}
      ...
      {/* {showadd && (
        <AddEmp forpopup={() => setshowadd(false)} heading="Add Leader" />
      )}
      {showadd && (
        <AddEmp forpopup={() => setshowadd(false)} heading="Add Team Member" />
      )} */}
    </div>
  );
};
